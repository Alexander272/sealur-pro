    ...
    return (
        <Container title='Исходные данные для расчета'>
            <Line
                ...
                res={formatNumber(data.pressure)}
                units='МПа'
            />
            <Line
                title='Внешняя осевая сила (растягивающая - положительная, сжимающая - отрицательная)'
                res={formatNumber(data.axialForce)}
                units='Н'
            />
            <Line
                title='Внешний изгибающий момент'
                res={formatNumber(data.bendingMoment)}
                units='Н*мм'
            />
            <Line title='Расчетная температура' res={formatNumber(data.temp)} units='&#8451;' />
