...
const Flange: FC<Props> = ({ typeFlange, standarts, materials, register, control, setValue, errors }) => {
	const type = useWatch({
		control,
	...
	return (
		<Container title={`Исходные данные для фланца`}>
			...
			<Temp register={register} control={control} errors={errors} title='фланца' letter='ф' path={`flangeData`} />
			...
			{standartId === 'another' ? (
				...
				<FlangeDefSize standarts={stands} control={control} register={register} setValue={setValue} />
			)}
