...
const Gasket: FC<Props> = ({ gasket, env, control, register, setValue, errors }) => {
	...
	useEffect(() => {
		setValue('gasket.thickness', gasket.find(g => g.id === gasketId)?.thickness[0].toString() || '3')
		...
		<Container title='Исходные данные для прокладки'>
			...
			{gasketId === 'another' && (
				<Suspense fallback={<Loader background='fill' />}>
					<GasketData register={register} control={control} setValue={setValue} errors={errors} />
				</Suspense>
			)}
		</Container>
	)
