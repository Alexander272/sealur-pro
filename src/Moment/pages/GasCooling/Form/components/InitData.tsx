...
const Data: FC<Props> = ({ data, register, control, setValue, errors, tube, sec, fin, num }) => {
	...
	const hasFriction = useWatch({ control, name: 'hasFriction' })
	...
	useEffect(() => {
		const section = data.sectionExecution.find(s => s.id === sectionId) || ({} as ISectionExecution)
		setValue('section', section)
	}, [sectionId, data, setValue])
	useEffect(() => {
		const numbers = data.numberOfMoves.find(n => n.id === numberOfMovesId) || ({} as INumberOfMoves)
			...
			<div className={classes.line}>
				...
				<div className={classes['line-field']}>
					<Controller
						...
						control={control}
						render={({ field }) => (
							<Select value={field.value} onChange={field.onChange} disabled={fin.length < 2}>
								{fin.map(d => (
									<Option key={d.id} value={d.id}>
										{d.value}
									</Option>
			...
			<div className={classes.line}>
				...
				<div className={classes['line-field']}>
					<Controller
						...
						control={control}
						render={({ field }) => (
							<Select value={field.value} onChange={field.onChange} disabled={sec.length < 2}>
								{sec.map(d => (
									<Option key={d.id} value={d.id}>
										{d.value}
									</Option>
			...
			<div className={classes.line}>
				...
				<div className={classes['line-field']}>
					<Controller
						...
						control={control}
						render={({ field }) => (
							<Select value={field.value} onChange={field.onChange} disabled={tube.length < 2}>
								{tube.map(d => (
									<Option key={d.id} value={d.id}>
										{d.value}
									</Option>
