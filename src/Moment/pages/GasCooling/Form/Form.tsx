...
import { IAVOData, IFinningFactor, INumberOfMoves, ISectionExecution, ITubeLength } from '../../../types/device'
...
const FormFields: FC<Props> = ({ register, control, setValue, errors }) => {
	const { data: res, error } = useSWR<{ data: IAVOData }>('/sealur-moment/data/avo', ReadService.getData)
	...
	useEffect(() => {
		if (res && !devId) {
			if (res.data.devices) setValue('devId', res.data.devices[0].id)
			if (res.data.pressure) setValue('pressureId', res.data.pressure[0].id)
			if (res.data.tubeCount) setValue('tubeCountId', res.data.tubeCount[0].id)
		}
	}, [res, devId, setValue])
	...
	useEffect(() => {
		if (res && devId) {
			...
			const num = res.data.numberOfMoves.filter(t => t.devId === devId && t.countId === countId)
