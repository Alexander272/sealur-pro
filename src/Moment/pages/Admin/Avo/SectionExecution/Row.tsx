...
export const Row: FC<Props> = ({ data, devId }) => {
    ...
    } = useForm<ISectionExecution>({
        defaultValues: data,
    })
    const { mutate } = useSWRConfig()
    const { toggle, isOpen } = useModal()

    const saveHandler = async (row: ISectionExecution) => {
        const newData = {
            devId: devId,
            value: row.value,
        }
        try {
            await AdminService.update(`/sealur-moment/section-execution/${data.id}`, newData)
            mutate(["/sealur-moment/section-execution", [{ name: "devId", value: devId }]])
    ...
    const deleteHandler = async () => {
        try {
            await AdminService.delete(`/sealur-moment/section-execution/${data.id}`)
            mutate(["/sealur-moment/section-execution", [{ name: "devId", value: devId }]])
            reset({}, { keepDirty: false })
        } catch (error) {
            toast.error("Произошла ошибка")
