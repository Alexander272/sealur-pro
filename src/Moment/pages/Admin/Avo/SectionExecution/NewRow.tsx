...
export const NewRow: FC<Props> = ({ devId }) => {
    const { register, setValue, reset, handleSubmit } = useForm<ISectionExecution[]>()
    const { mutate } = useSWRConfig()

    const [newRowCount, setNewRowCount] = useState(0)

    const saveHandler = async (data: ISectionExecution[]) => {
        let arr = []
        ...
        try {
            await AdminService.create(`/sealur-moment/section-execution/few`, arr)
