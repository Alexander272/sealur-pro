...
export const NewRow: FC<Props> = () => {
    const { register, setValue, reset, handleSubmit } = useForm<IPressure[]>()
    const { mutate } = useSWRConfig()

    const [newRowCount, setNewRowCount] = useState(0)

    const saveHandler = async (data: IPressure[]) => {
        let arr = []
        ...
        try {
            await AdminService.create(`/sealur-moment/pressure/few`, arr)
