...
export const Row: FC<Props> = ({ data }) => {
    ...
    const saveHandler = async (row: ITubeCount) => {
        const newData = {
            value: +row.value,
        }
        try {
            await AdminService.update(`/sealur-moment/tube-count/${data.id}`, newData)
    ...
    const deleteHandler = async () => {
        try {
            await AdminService.delete(`/sealur-moment/tube-count/${data.id}`)
            mutate(`/sealur-moment/tube-count`)
            reset({}, { keepDirty: false })
        } catch (error) {
            toast.error("Произошла ошибка")
