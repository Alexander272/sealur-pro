...
export const NewRow: FC<Props> = ({ devId }) => {
    const { register, setValue, reset, handleSubmit } = useForm<IFinningFactor[]>()
    const { mutate } = useSWRConfig()

    const [newRowCount, setNewRowCount] = useState(0)

    const saveHandler = async (data: IFinningFactor[]) => {
        let arr = []
        ...
        try {
            await AdminService.create(`/sealur-moment/finning-factor/few`, arr)
