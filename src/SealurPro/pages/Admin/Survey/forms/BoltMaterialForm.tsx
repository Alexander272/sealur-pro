...
export const BoltMaterialForm: FC<Props> = ({ data, closeHandler, sendHandler }) => {
    const materials = useSelector((state: ProState) => state.survey.boltMaterials)
    ...
    const submitHandler = async (form: IMaterial) => {
        ...
        try {
            sendHandler()
            if (!data) {
                const res = await SurveyAdminService.createMaterials({ ...form, typeMat: "bolt" })
            ...
            survey.setBoltMaterials(newMat)
